...
// Snow Effect
// snow.js - Complete snowfall effect with even distribution
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Snow configuration - easily customizable
    const config = {
        density: 200,
        speed: 5,
        size: 8,
    ...
    function createSnowflakes() {
        ...
        for (let i = 0; i < config.density; i++) {
            ...
            // Random properties with even distribution
            const snowflakeSize = Math.random() * config.size + 2;
            const startPositionX = Math.random() * window.innerWidth;
            const startPositionY = Math.random() * -window.innerHeight;
            const opacity = Math.random() * 0.7 + 0.3;
            const fallSpeed = Math.random() * config.speed + 1;
            const swayAmplitude = Math.random() * config.swayAmplitude + 10;
            const swaySpeed = Math.random() * config.swaySpeed + 0.005;
            ...
            snowflakes.push({
                ...
                originalX: startPositionX // Store original position for reference
            });
    ...
    function animateSnow() {
        ...
        snowflakes.forEach(snowflake => {
            ...
            // Balanced horizontal movement with sine wave
            const horizontalMovement = Math.sin(time * snowflake.swaySpeed + snowflake.swayOffset) * 
                                     snowflake.swayDirection * 
                                     snowflake.sway * 0.5;
            
            // Calculate new position while maintaining overall distribution
            snowflake.x = snowflake.originalX + horizontalMovement;
            
            // Reset if out of view at bottom
            ...
            if (snowflake.x > window.innerWidth) {
                snowflake.x = 0;
                snowflake.originalX = 0;
            }
            if (snowflake.x < 0) {
                snowflake.x = window.innerWidth;
                snowflake.originalX = window.innerWidth;
            }
    ...
    // Public API (optional - if you want to control snow from other scripts)
    window.snowEffect = {
        start: startSnow,
        stop: stopSnow,
